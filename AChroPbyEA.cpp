...
typechrdivider chrdivider[23][20];
...
int minCM = 27;              /* Minimum chunk length in cM for hotspot segments. User-configurable via minCM=N. */
...
typedef struct {
	double minCM;
	float seuiltocorrect;
	float seuilpihat;
	double ratetoconsiderseg;
	double ratetoconsidersegPE;
	float firstexponent;
	int powersnppihat;
	float thirdindice;
	float penalty;
	float mergingexponent0;
	float exponantnumer;
	float pihat2max;
	int nbsnpminforseg;
	float powermaf;
...
/* Compute chrdivider from hotspots (R logic): top 10 per chr (fixed), merge if gap < minCM, map bp->SNP index. */
static int fill_chrdivider_from_hotspots(hotspot_t *all, int ntotal, double minCM) {
	int *map_bp = (int *)malloc(NSNPPERCHR * sizeof(int));
	if (!map_bp) return -1;
	nbchrdivider[0] = 0;
	static const int MAX_BREAKS = 10;
	...
	for (int chr = 1; chr <= 22; chr++) {
		...
		if (nsub == 0) {
			chrdivider[chr][0].start = 0;
			chrdivider[chr][0].end = nbsnpperchr[chr];
			chrdivider[chr][0].segment = 0;
			nbchrdivider[chr] = 1;
			if (1 > nbchrdivider[0]) nbchrdivider[0] = 1;
			continue;
		}
		qsort(sub, nsub, sizeof(hotspot_t), hotspot_cmp_cMperMb);
		nsub = (nsub > MAX_BREAKS) ? MAX_BREAKS : nsub;
		...
		for (int i = 1; i < nm; i++) {
			...
			chrdivider[chr][nseg].start = start;
			chrdivider[chr][nseg].end = end_ix;
			chrdivider[chr][nseg].segment = 0;
			start = end_ix;
			nseg++;
		...
		if (start < n_map) {
			chrdivider[chr][nseg].start = start;
			chrdivider[chr][nseg].end = n_map;
			chrdivider[chr][nseg].segment = 0;
			nseg++;
		}
		nbchrdivider[chr] = nseg;
		if (nseg > nbchrdivider[0]) nbchrdivider[0] = nseg;
	}
...
{	FILE * fileresult;
	...
	{	int tok_count = 0, in_token = 0, c;
		...
		if (tok_count >= 6) n_indiv = (tok_count - 6) / 2;  /* 6 fixed: (NA/)chr rsid pos ref alt, or 5 for legacy */
		else if (tok_count >= 5) n_indiv = (tok_count - 5) / 2;
	}
	{	int c;
	...
	*out_buf = (unsigned char*)calloc(1, buf_size);
	...
	{	int dummy_chr, dummy_pos;
		...
		if (strcmp(first_col, "NA") == 0) {
			if (fscanf(fileresult, " %d %127s %d %c %c", &dummy_chr, rsid, &dummy_pos, &ref, &alt) < 5) break;
		} else {
			dummy_chr = atoi(first_col);
...
int acrosschrphasing(int ID,int IDp1loop,int IDp2loop,char pathresult[], const char *pathHapDir, const char *pathwindow,
	...
	double (*pihatagainstallchrMPphaseerror)[MAXPOP][200], const phasing_params_t *params)
{
	printf("Start main phasing program (minCM=%g)\n", params->minCM);
	fflush(stdout);
	int nind = n_individuals > 0 ? n_individuals : MAXPOP;
	seuilpihat[0]=params->seuilpihat;
	seuilpihat[1]=params->seuilpihat;
	seuilpihat[2]=params->seuilpihat;
	/************************************************************************************************ */
	...
	printf("\nTotal time loading focal and paretns:%f cpu click so %f seconds\n",elapsed_secs11,elapsed_secs11/CLOCKS_PER_SEC );
	/************************************************************************************************ */
	/*									iniatial variables											  */
	/************************************************************************************************ */
	int64_t iter=0;
	...
	int powerpihatDEGREE4=0;
		...
		/************************************************************************************************ */
	/*									Initiate segments from recombinasion maps					  */
	/************************************************************************************************ */
	for(int chr=1;chr<23;chr++)
	chrdivider[chr][0].segment=0;

	if (pathHotspots[0] && pathMap[0]) {
		/* Hotspot-based segments: from file only. pathMap used for SNP indexing. */
		...
		printf("Segment boundaries from hotspot file: %s (pathMap=%s minCM=%g)\n", pathHotspots, pathMap, params->minCM);
		if (fill_chrdivider_from_hotspots(all, ntotal, params->minCM) != 0) {
		...
		/* Window segment boundaries: read from pathwindow */
		int CHR27_BOUNDS[23][5];
		{
			...
			for (int chr = 0; chr < 23; chr++) {
				if (!fgets(line, sizeof(line), fb)) {
					...
					fprintf(stderr, "ERROR: %s too short (need 23 lines)\n", pathwindow);
					fclose(fb);
					exit(1);
				}
				while (line[0] == '#' || line[0] == '\n')
					{ if (!fgets(line, sizeof(line), fb)) { printf("EXIT(1): %s read error (chr %d)\n", pathwindow, chr); fclose(fb); exit(1); } }
		...
		printf("Segment boundaries from window file: %s\n", pathwindow);
		nbchrdivider[0] = 0;
		for(int chr=1; chr<=22; chr++) {
			int start = 0, n = 0;
			for(int i=0; i<5; i++) {
				...
				if (b < 0) {
					chrdivider[chr][n].start = start;
					chrdivider[chr][n].end = nbsnpperchr[chr];
					n++;
					break;
				}
				chrdivider[chr][n].start = start;
				chrdivider[chr][n].end = b;
				start = b;
				n++;
			}
			nbchrdivider[chr] = n;
			if (n > nbchrdivider[0]) nbchrdivider[0] = n;
		}
	} else {
		printf("EXIT(1): Segment boundaries required. Provide pathwindow= OR (pathHotspots= + pathMap=).\n");
		fprintf(stderr, "ERROR: Need window file (pathwindow) or both hotspot file (pathHotspots) and map (pathMap).\n");
		exit(1);
	}

	printf("\n--- Window limits per chromosome (SNP indices, 0-based) ---\n");
	for (int chr = 1; chr <= 22; chr++) {
		int nseg = nbchrdivider[chr];
		printf("chr %2d (%d windows):", chr, nseg);
		for (int s = 0; s < nseg; s++) {
			printf(" [%d-%d]", chrdivider[chr][s].start, chrdivider[chr][s].end);
		}
		printf("\n");
	}
	printf("----------------------------------------------------------\n\n");
	...
	float seuiltocorrect = (float)params->seuiltocorrect;
...
///////////******************************************************************************************************************************************************************************			
	...
	{	int countsnp=0;
		...
		if (pihatagainstall[IDrelattocompare]>seuiltocorrect || relattocompare==0)
		{		
...
//#pragma omp parallel for 	
			...
			{	printf("Start correcting chr %d with %d nb of SNPs\n",chrtemp1,nbsnpperchr[chrtemp1]);
				...
				double ratetoconsiderseg = params->ratetoconsiderseg;
				double ratetoconsidersegPE = params->ratetoconsidersegPE;  
				int phaseerrorpossible[4]={0};
				int nbsnpminforseg = params->nbsnpminforseg;
				for(int snp=0;snp<nbsnpperchr[chrtemp1];snp++)
				{//	printf("SNP	%d strat",snp);
					...
					{	int snpvalue1=(*((genomes[chrtemp1]+(unsigned long long) relat*nbsnpperchrby4 )+snpdiv4)>>snpmod4)&3;
						...
						{	if (nbindivmatchseg[relat][0]<nbsnpminforseg) nbindivmatchseg[relat][0]++; else 
							nbsegmentofthislength[++nbindivmatchseg[relat][0]]++;
						} else 
						{	if (nbindivmatchseg[relat][0]<nbsnpminforseg) nbindivmatchseg[relat][0]=0; else 
							{	for(int lenght=nbsnpminforseg;lenght<nbindivmatchseg[relat][0]+1;lenght++) nbsegmentofthislength[lenght]--;
								nbindivmatchseg[relat][0]=0;
							}
						};
						if ((snpvalue0&1)==(snpvalue1>>1))
						{	if (nbindivmatchseg[relat][1]<nbsnpminforseg) nbindivmatchseg[relat][1]++;
							else nbsegmentofthislength[++nbindivmatchseg[relat][1]]++;
						} else 
						{	if (nbindivmatchseg[relat][1]<nbsnpminforseg) nbindivmatchseg[relat][1]=0; else 
							{	for(int lenght=nbsnpminforseg;lenght<nbindivmatchseg[relat][1]+1;lenght++) nbsegmentofthislength[lenght]--;
								nbindivmatchseg[relat][1]=0;
							};
						};
						if ((snpvalue0>>1)==(snpvalue1&1))
						{	if (nbindivmatchseg[relat][2]<nbsnpminforseg) nbindivmatchseg[relat][2]++;
							else nbsegmentofthislength[++nbindivmatchseg[relat][2]]++;
						} else 
						{	if (nbindivmatchseg[relat][2]<nbsnpminforseg) nbindivmatchseg[relat][2]=0;
							else 
							{	for(int lenght=nbsnpminforseg;lenght<nbindivmatchseg[relat][2]+1;lenght++) nbsegmentofthislength[lenght]--;
								nbindivmatchseg[relat][2]=0;
							}
						};
						if ((snpvalue0>>1)==(snpvalue1>>1))
						{	if (nbindivmatchseg[relat][3]<nbsnpminforseg) nbindivmatchseg[relat][3]++;
							else nbsegmentofthislength[++nbindivmatchseg[relat][3]]++;
						} else 
						{	if (nbindivmatchseg[relat][3]<nbsnpminforseg) nbindivmatchseg[relat][3]=0;	
							else 
							{	for(int lenght=nbsnpminforseg;lenght<nbindivmatchseg[relat][3]+1;lenght++) nbsegmentofthislength[lenght]--;
								nbindivmatchseg[relat][3]=0;	
							};
						};
					};
...
//	printf("FROM PARENT pourcentage before %d %f\n",purcentage,(float) bestpur/330005); 
	...
	{	//for(int  snpphaseerror=0;snpphaseerror<nbsnpperchr[chrtemp1];snpphaseerror=snpphaseerror+100)		
		for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1];chrdividerrun++)
	...
	{	//for(int  snpphaseerror=0;snpphaseerror<nbsnpperchr[chrtemp1];snpphaseerror=snpphaseerror+100)		
		...
		{	int nbphaseright=0;
			int nbphasewrong=0;
			for(int snp=chrdivider[chrtemp1][chrdividerrun].start;snp<chrdivider[chrtemp1][chrdividerrun].end;snp++)		
...
#pragma omp parallel for					
	for(int relat=0;relat<nind;relat++)
	{	pihatagainstallchrMPphaseerror[0][relat][0]=0;
		for(int chrdividerrun=0;chrdividerrun<nbchrdivider[0]*4;chrdividerrun++)
			pihatagainstallchrMPphaseerror[0][relat][chrdividerrun]=0;
	}
	int hetsnp=0;
	double exponentinterchr=1;
	

	float firstexponent = params->firstexponent;

	int powersnppihat = params->powersnppihat;
	for(int  chrtemp1=1;chrtemp1<23;chrtemp1++)		
	{	//for(int  snpphaseerror=0;snpphaseerror<nbsnpperchr[chrtemp1];snpphaseerror=snpphaseerror+100)		
		{	hetsnp=0;
...
#pragma omp parallel for 				
			for(int relat=0;relat<nind;relat++)
			{	for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1]*4;chrdividerrun++)
					pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun]=0;
			};
			...
			{	double (*temp)[4] = (double (*)[4])calloc(nind, sizeof(double[4]));
				...
				{	int snpvalue0=genomeoffpss[0][snp][chrtemp1];
					...
					{//	if (snp<snpphaseerror) snpvalue0=3-snpvalue0;
						...
						double maffloatdiviseur=pow((maffloat)*(2-maffloat*2)/3*2,1.0/params->powermafdiv);
						maffloat=pow(maffloat,1.0/params->powermaf);
						...
						double pconttab[4][2];
						pconttab[0][0]=pow(((0)-maffloat)*pcontribu10/maffloatdiviseur,1.0/powersnppihat);
						pconttab[0][1]=pow(((0)-maffloat)*pcontribu11/maffloatdiviseur,1.0/powersnppihat);
						pconttab[1][0]=pow(((1)-maffloat)*pcontribu10/maffloatdiviseur,1.0/powersnppihat);
						pconttab[1][1]=pow(((1)-maffloat)*pcontribu11/maffloatdiviseur,1.0/powersnppihat); 
						int nbsnpperchrby4=(nbsnpperchr[chrtemp1]/4+((nbsnpperchr[chrtemp1]%4)>0));
						int snpmod4=(((snp%4)*2));
						int snpdiv4=snp/4;
#pragma omp parallel for
						for(int64_t relat=0;relat<(int64_t) nind;relat++) if (pihatagainstall[relat]<seuilpihat[0] && pihatagainstall2[relat]<params->pihat2max)
						{	int snpvalue1=(*((genomes[chrtemp1]+(unsigned long long) relat*nbsnpperchrby4 )+snpdiv4)>>snpmod4)&3;
							if (pconttab[snpvalue1&1][0]>0) temp[relat][0]=temp[relat][0]+(pconttab[snpvalue1&1][0]);
							else temp[relat][0]=0;
							if (temp[relat][0]<0) temp[relat][0]=0;
							else if (pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]<temp[relat][0]) pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]=temp[relat][0];
							if (pconttab[snpvalue1>>1][0]>0) temp[relat][1]=temp[relat][1]+(pconttab[snpvalue1>>1][0]);
							else temp[relat][1]=0;
							if (temp[relat][1]<0) temp[relat][1]=0;
							else if (pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+1]<temp[relat][1]) pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+1]=temp[relat][1];
							if (pconttab[snpvalue1&1][1]>0) temp[relat][2]=temp[relat][2]+(pconttab[snpvalue1&1][1]);
							else temp[relat][2]=0;
							if (temp[relat][2]<0) temp[relat][2]=0;
							else if (pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]<temp[relat][2]) pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]=temp[relat][2];
							if (pconttab[snpvalue1>>1][1]>0) temp[relat][3]=temp[relat][3]+(pconttab[snpvalue1>>1][1]);
							else temp[relat][3]=0;
							if (temp[relat][3]<0) temp[relat][3]=0;
							else if (pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+3]<temp[relat][3]) pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+3]=temp[relat][3];

						};
					};
				};
				for(int relat=0;relat<nind;relat++) 
				{	pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]=
					(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]>pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+1]?
					pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]:
					pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+1]);
					pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]=
					(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]>pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+3]?
					pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]:
					pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+3]);
				};
...
///////////******************************************************************************************************************************************************************************			
	...
	float thirdindice = params->thirdindice;
	double corglobmax=0;						
	int chrmax1;						
	int chrmax2;						
	int dividmax1;						
	int dividmax2;						
	double sumall[23][25][2];
	double sumallsquare[23][25][2];
#pragma omp parallel for 
	for(int  chrtemp1=1;chrtemp1<23;chrtemp1++)		
	{	for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1];chrdividerrun++)
		{	sumall[chrtemp1][chrdividerrun][0]=0;
			sumall[chrtemp1][chrdividerrun][1]=0;
			sumallsquare[chrtemp1][chrdividerrun][0]=0;
			sumallsquare[chrtemp1][chrdividerrun][1]=0;
			for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
			{	sumall[chrtemp1][chrdividerrun][0]=sumall[chrtemp1][chrdividerrun][0]+
				pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]),firstexponent)*
				(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]>0?1:-1);
				sumall[chrtemp1][chrdividerrun][1]=sumall[chrtemp1][chrdividerrun][1]+
				pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]),firstexponent)*
				(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]>0?1:-1);
				sumallsquare[chrtemp1][chrdividerrun][0]=sumallsquare[chrtemp1][chrdividerrun][0]+
				pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]),firstexponent*2);
				sumallsquare[chrtemp1][chrdividerrun][1]=sumallsquare[chrtemp1][chrdividerrun][1]+
				pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]),firstexponent*2);
			}
		};
	};
...
#pragma omp parallel for		
	...
	{	for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1];chrdividerrun++)
		{	for(int  chrtemp2=chrtemp1;chrtemp2<23;chrtemp2++)		
			{	for(int chrdividerrun2=0;chrdividerrun2<nbchrdivider[chrtemp2];chrdividerrun2++) 
...
#pragma omp parallel for
	...
	{	for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1];chrdividerrun++)
		{	for(int  chrtemp2=chrtemp1;chrtemp2<23;chrtemp2++)		
			{	for(int chrdividerrun2=0;chrdividerrun2<nbchrdivider[chrtemp2];chrdividerrun2++) if (chrtemp1!=chrtemp2 || chrdividerrun<chrdividerrun2)
				{	double sumproduct; 
					sumproduct=0;
					double nbelem=0;
					...
					{	nbelem++;
						...
						pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]),firstexponent)*
						pow(fabs(pihatagainstallchrMPphaseerror[chrtemp2][relat][chrdividerrun2*4]),firstexponent);
					};
					double  cor1=(((nbelem)*sumproduct-sumall[chrtemp1][chrdividerrun][0]*sumall[chrtemp2][chrdividerrun2][0])/
					sqrt(((nbelem)*sumallsquare[chrtemp1][chrdividerrun][0]-sumall[chrtemp1][chrdividerrun][0]*sumall[chrtemp1][chrdividerrun][0])*
					((nbelem)*sumallsquare[chrtemp2][chrdividerrun2][0]-sumall[chrtemp2][chrdividerrun2][0]*sumall[chrtemp2][chrdividerrun2][0])));
					sumproduct=0;
					nbelem=0;
					for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
					{	nbelem++;
						sumproduct=sumproduct+pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4]),firstexponent)*
						pow(fabs(pihatagainstallchrMPphaseerror[chrtemp2][relat][chrdividerrun2*4+2]),firstexponent);
					};
					double  cor2=(((nbelem)*sumproduct-sumall[chrtemp1][chrdividerrun][0]*sumall[chrtemp2][chrdividerrun2][1])/
					sqrt(((nbelem)*sumallsquare[chrtemp1][chrdividerrun][0]-sumall[chrtemp1][chrdividerrun][0]*sumall[chrtemp1][chrdividerrun][0])*
					((nbelem)*sumallsquare[chrtemp2][chrdividerrun2][1]-sumall[chrtemp2][chrdividerrun2][1]*sumall[chrtemp2][chrdividerrun2][1])));
					sumproduct=0;
					nbelem=0;
					for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
					{	nbelem++;
						sumproduct=sumproduct+pow(fabs(pihatagainstallchrMPphaseerror[chrtemp1][relat][chrdividerrun*4+2]),firstexponent)*
						pow(fabs(pihatagainstallchrMPphaseerror[chrtemp2][relat][chrdividerrun2*4+2]),firstexponent);
					};
					double  cor3=(((nbelem)*sumproduct-sumall[chrtemp1][chrdividerrun][1]*sumall[chrtemp2][chrdividerrun2][1])/
					sqrt(((nbelem)*sumallsquare[chrtemp1][chrdividerrun][1]-sumall[chrtemp1][chrdividerrun][1]*sumall[chrtemp1][chrdividerrun][1])*
					((nbelem)*sumallsquare[chrtemp2][chrdividerrun2][1]-sumall[chrtemp2][chrdividerrun2][1]*sumall[chrtemp2][chrdividerrun2][1])));
					sumproduct=0;
					nbelem=0;
					for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
					...
					double  cor4=(((nbelem)*sumproduct-sumall[chrtemp1][chrdividerrun][1]*sumall[chrtemp2][chrdividerrun2][0])/
					sqrt(((nbelem)*sumallsquare[chrtemp1][chrdividerrun][1]-sumall[chrtemp1][chrdividerrun][1]*sumall[chrtemp1][chrdividerrun][1])*
					((nbelem)*sumallsquare[chrtemp2][chrdividerrun2][0]-sumall[chrtemp2][chrdividerrun2][0]*sumall[chrtemp2][chrdividerrun2][0])));
					...
					{	corglobmax=corglob;
						...
						/*printf("%d %d %d %d %f %f %f %f %f %f %f %f %f %f %f %f %f  \n",chrtemp1,chrtemp2,chrdividerrun,chrdividerrun2,cor1,cor2,cor3,cor4,
							sumproduct,nbelem,
							sumallsquare[chrtemp1][chrdividerrun][0],sumallsquare[chrtemp2][chrdividerrun2][0],
							sumall[chrtemp1][chrdividerrun][0],sumall[chrtemp2][chrdividerrun2][0],
...
#pragma omp parallel for 	
	...
	int nbmerge=0;
	float mergingexponent0 = params->mergingexponent0;
	int nbpair=0;
	do 
...
#pragma omp parallel for		
			for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
			{	double p1=pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]>0?1:-1);
				double p2=pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]>0?1:-1);
				double p3=pow(fabs(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4]>0?1:-1);
				double p4=pow(fabs(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4+2]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4+2]>0?1:-1);
				pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]=pow(fabs(p1+p3),1.0/mergingexponent0)*(p1+p3>0?1:-1);
				pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]=pow(fabs(p2+p4),1.0/mergingexponent0)*(p2+p4>0?1:-1);
			}
...
#pragma omp parallel for		
			...
			{	double p1=pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]>0?1:-1);
				...
				double p3=pow(fabs(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4]>0?1:-1);
				double p4=pow(fabs(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4+2]),mergingexponent0)*(pihatagainstallchrMPphaseerror[chrmax2][relat][dividmax2*4+2]>0?1:-1);
				pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]=pow(fabs(p1+p4),1.0/mergingexponent0)*(p1+p4>0?1:-1);
				pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]=pow(fabs(p2+p3),1.0/mergingexponent0)*(p2+p3>0?1:-1);
			};					
		};
///////////******************************************************************************************************************************************************************************
...
///////////******************************************************************************************************************************************************************************			
		...
		sumall[chrmax1][dividmax1][0]=0;
		sumall[chrmax1][dividmax1][1]=0;
		sumallsquare[chrmax1][dividmax1][0]=0;
		sumallsquare[chrmax1][dividmax1][1]=0;
		for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
		{	sumall[chrmax1][dividmax1][0]=sumall[chrmax1][dividmax1][0]+
			pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]),firstexponent)*
			(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]>0?1:-1);
			sumall[chrmax1][dividmax1][1]=sumall[chrmax1][dividmax1][1]+
			pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]),firstexponent)*
			(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]>0?1:-1);
			sumallsquare[chrmax1][dividmax1][0]=sumallsquare[chrmax1][dividmax1][0]+ pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4]),firstexponent*2);
			sumallsquare[chrmax1][dividmax1][1]=sumallsquare[chrmax1][dividmax1][1]+ pow(fabs(pihatagainstallchrMPphaseerror[chrmax1][relat][dividmax1*4+2]),firstexponent*2);
		};
		...
		{	for(int chrdividerrun2=0;chrdividerrun2<nbchrdivider[chrtemp2];chrdividerrun2++) if (havemerged[chrtemp2][chrdividerrun2]==0 && (chrtemp1!=chrtemp2 || chrdividerrun!=chrdividerrun2))
			{	double sumproduct; 
				sumproduct=0;
				int nbelem=0;
				for(int relat=0;relat<nind;relat++) if (pihatagainstall[relat]<seuilpihat[0])
				...
				float exponantnumer = params->exponantnumer;
				double corglob=(pow(fabs(cor1),thirdindice-1)*cor1 -
				pow(fabs(cor2),thirdindice-1)*cor2+ 
				pow(fabs(cor3),thirdindice-1)*cor3-
				pow(fabs(cor4),thirdindice-1)*cor4)
...
///////////******************************************************************************************************************************************************************************			
		...
		for(int  chrtemp1=1;chrtemp1<23;chrtemp1++)		
		{	for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1];chrdividerrun++) if (havemerged[chrtemp1][chrdividerrun]==0)
			{	for(int  chrtemp2=chrtemp1;chrtemp2<23;chrtemp2++)		
				{	for(int chrdividerrun2=0;chrdividerrun2<nbchrdivider[chrtemp2];chrdividerrun2++) if (havemerged[chrtemp2][chrdividerrun2]==0 && (chrtemp1!=chrtemp2 || chrdividerrun<chrdividerrun2))
					{	nbpair++;
						...
						//	printf("NB1 %d MB2 %d c %f \n",nbingroup[chrtemp1][chrdividerrun],nbingroup[chrtemp2][chrdividerrun2],corglob);  
							...
							//	printf("1 is %d %d\n",chrnotone,chrdividerrunnotone);
								...
								for(int chrdividerrunrun=0;chrdividerrunrun<nbchrdivider[chrone];chrdividerrunrun++) 
		...
		for(int  chr=1;chr<23;chr++)		
		{	//printf("%d ",chr);
			for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chr];chrdividerrun++)
...
///////////******************************************************************************************************************************************************************************			
	...
	{	for(int chrdividerrun=0;chrdividerrun<nbchrdivider[chrtemp1];chrdividerrun++)
		...
		//	printf("pahse chr %d div %d is %d\n",chrtemp1,chrdividerrun,phaseknown);
			chrdivider[chrtemp1][chrdividerrun].phasing=phaseknown;
			...
			{	if ((phaseknown+3)/2==2) 
				...
				{	nbphaseright++;
					...
					chrdivider[0][0].nbright++;
					chrdivider[chrtemp1][chrdividerrun].nbright++;
				} else 
				{	nbphasewrong++;
					phase2perchr[chrtemp1]++;
					chrdivider[0][0].nbwrong++;
					chrdivider[chrtemp1][chrdividerrun].nbwrong++;
...
static void parse_arg(const char *arg, char *pathresult, phasing_params_t *p)
{
	...
	if (strncmp(arg, "minCM=", 6) == 0 && p) { double v = atof(val); p->minCM = (v > 0) ? v : 27; return; }
	if (strncmp(arg, "seuiltocorrect=", 15) == 0 && p) { p->seuiltocorrect = (float)atof(val); return; }
	if (strncmp(arg, "seuilpihat=", 11) == 0 && p) { p->seuilpihat = (float)atof(val); return; }
	if (strncmp(arg, "ratetoconsiderseg=", 18) == 0 && p) { p->ratetoconsiderseg = atof(val); return; }
	if (strncmp(arg, "ratetoconsidersegPE=", 19) == 0 && p) { p->ratetoconsidersegPE = atof(val); return; }
	if (strncmp(arg, "firstexponent=", 14) == 0 && p) { p->firstexponent = (float)atof(val); return; }
	if (strncmp(arg, "powersnppihat=", 14) == 0 && p) { p->powersnppihat = atoi(val); return; }
	if (strncmp(arg, "thirdindice=", 12) == 0 && p) { p->thirdindice = (float)atof(val); return; }
	if (strncmp(arg, "penalty=", 8) == 0 && p) { p->penalty = (float)atof(val); return; }
	if (strncmp(arg, "mergingexponent0=", 17) == 0 && p) { p->mergingexponent0 = (float)atof(val); return; }
	if (strncmp(arg, "exponantnumer=", 14) == 0 && p) { p->exponantnumer = (float)atof(val); return; }
	if (strncmp(arg, "pihat2max=", 10) == 0 && p) { p->pihat2max = (float)atof(val); return; }
	if (strncmp(arg, "nbsnpminforseg=", 15) == 0 && p) { p->nbsnpminforseg = atoi(val); return; }
	if (strncmp(arg, "powermaf=", 9) == 0 && p) { p->powermaf = (float)atof(val); return; }
	if (strncmp(arg, "powermafdiv=", 12) == 0 && p) { p->powermafdiv = (float)atof(val); return; }
...
int main(int argc, char *argv[])
{
	...
	phasing_params_t params = {
		.minCM = 27,
		.seuiltocorrect = 0.022f,
		.seuilpihat = 0.33f,
		.ratetoconsiderseg = 0.000012,
		.ratetoconsidersegPE = 0.000012,
		.firstexponent = 2.f,
		.powersnppihat = 10,
		.thirdindice = 1.f,
		.penalty = 0.75f,
		.mergingexponent0 = 3.f,
		.exponantnumer = 0.5f,
		.pihat2max = 0.009f,
		.nbsnpminforseg = 10,
		.powermaf = 1.f,
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "help") == 0 || strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
			...
			printf("Other: posOffspring=N posParent1=N posParent2=N minCM=N (default 27)\n");
			printf("Phasing params: seuiltocorrect= (0.022) seuilpihat= (0.33) ratetoconsiderseg= (0.000012) ratetoconsidersegPE= (0.000012) firstexponent= (2) powersnppihat= (10) thirdindice= (1) penalty= (0.75) mergingexponent0= (3) exponantnumer= (0.5) pihat2max= (0.009) nbsnpminforseg= (10) powermaf= (1) powermafdiv= (1)\n");
		...
		parse_arg(argv[i], pathresult, &params);
	}
	if (pathResult[0] == '\0') strncpy(pathResult, pathresult, sizeof(pathResult)-1);

	printf("pathHap=%s pathHotspots=%s pathMap=%s pathResult=%s pathwindow=%s useBinary=%d minCM=%g posOffspring=%d posParent1=%d posParent2=%d\n",
		pathHap, pathHotspots, pathMap, pathResult, pathwindow, useBinary, params.minCM, posOffspring, posParent1, posParent2);
	...
	printf("Loading HAP files (chr 1-22)...\n");
	fflush(stdout);
	for(int chrtemp1=1;chrtemp1<23;chrtemp1++)
	...
	double (*pihatagainstallchrMPphaseerror)[MAXPOP][200] = (double (*)[MAXPOP][200])calloc(23, sizeof(double[MAXPOP][200]));
	...
	if (!pihatagainstallchrMPphaseerror) {
		printf("RETURN(1): Failed to allocate pihatagainstallchrMPphaseerror (~16 GB) in main\n");
		free(phaseparent1tap); free(phaseguesedparent1tap); free(pihatagainstallchrMPphaseerror);
		return 1;
	}
	...
	int phasing_ret = acrosschrphasing(ID, IDp1, IDp2, pathresult, pathHap, pathwindow,
		phaseparent1tap, phaseguesedparent1tap, pihatagainstallchrMPphaseerror, &params);
